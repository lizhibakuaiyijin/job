...
<head>
    ...
    window.onload = function() {
        if ($('input[name="jobCater"]:checked').val() == 2) { // 兼职
            $(".partTime input.wage").val("<?php echo ($info['pay']); ?>");
            ...
            $(".fullTime input.wage").val("<?php echo ($info['pay']); ?>");
            // 切换
...
<body>
    ...
    <form class="form-horizontal addwrap" enctype="multipart/form-data">
        <div class="form-group jobName">
            ...
            <div class="col-sm-10">
                <input type="text" class="form-control jobName" name="jobName" placeholder="请输入职位名称" value="<?php echo ($info['post_name']); ?>">
                <input type="hidden" name="post_id" value="<?php echo ($info['post_id']); ?>" id="post_id">
            </div>
        ...
        <div class="form-group jobArea">
            ...
            <div class="col-sm-10">
                <select id="province">
                    <?php if(is_array($area)): foreach($area as $key=>$v): ?><option value="<?php echo ($v['province_id']); ?>" <?php if($v[ 'province_id']==$info[ 'province_id']) echo 'selected'?>><?php echo ($v['name']); ?></option><?php endforeach; endif; ?>
                </select>
                <select id="city">
                    <?php if(is_array($info['city_info'])): foreach($info['city_info'] as $key=>$c): ?><option value="<?php echo ($c['city_id']); ?>" <?php if($c[ 'city_id']==$info[ 'city_id']) echo 'selected'?>><?php echo ($c['name']); ?></option><?php endforeach; endif; ?>
                </select>
                <select id="district">
                    <?php if(is_array($info['district_info'])): foreach($info['district_info'] as $key=>$d): ?><option value="<?php echo ($d['district_id']); ?>" <?php if($d[ 'district_id']==$info[ 'district_id']) echo 'selected'?>><?php echo ($d['name']); ?></option><?php endforeach; endif; ?>
                </select>
            </div>
            <div>
                <label class="col-sm-2"></label>
        ...
        <div class="form-group jobAddress">
            ...
            <div class="col-sm-10">
              <input type="text" class="form-control" value="<?php echo ($info['address']); ?>" placeholder="请输入工作地点">
            </div>
            <div>
                <label class="col-sm-2"></label>
                <div class="col-sm-2 notice"></div>
        ...
        <div class="form-group jobDetail">
            ...
            <div class="col-sm-10">
                <textarea id="detail" placeholder="请描述职位"><?php echo ($info['post_detail']); ?></textarea>
            </div>
        </div>
        <!--职位详情-->
    ...
    $("#submit").click(function() {
        ...
        if (jobName == true && jobType == true && jobCater == true && jobWage == true && jobTime == true && jobAddress == true && jobArea == true) {
            ...
            post_id = $('#post_id').val();
            $.ajax({
                url: "<?php echo U('post/post_update');?>",
                type: "post",
                dataType: "json",
                data: {
                    post_id,
